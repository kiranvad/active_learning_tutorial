...
from tutlib.plot import (plot_ternary, make_boundary_plots, make_score_plots, )
from tutlib.util import (composition_grid_ternary, calculate_perimeter_score_v2, trace_boundaries)
...
def actively_learn(input_dataset: xr.Dataset, niter: int, num_phases: int, label: Callable, extrapolate: Callable,
    ...
    input_dataset: xr.Dataset
        Starting dataset for active learning run. Should have measurement (sas) and composition (c,a,b) data variables

    niter: int
        Number of iterations to run active learning for

    num_phases: int
        Number of phases to search for

    label: Callable
        Method used to "label" or classify data. This method will be called first in the active learning loop

    extrapolate: Callable
        Method used to extrapolate/interpolate labels from specific points to entire composition space. This method is
        called second in the active learning loop

    choose_next_acquisition: Callable
        Method used to determine next composition to measure. Commonly callled the "acquisition function". This method is
        called last int he active learning loop

    instrument: VirtualSAS
        Virtual instrument object

    grid_pts_per_row: int
        Number of gridpoints in each dimension in ternary (Barycentric) space

    plot: bool
        Whether or not to generate plots. Note that these plots can significantly slow down the loop.

    plot_every: int
        Frequency of plot refreshes

    plot_skip_phases: List[str]
        Which phases to exclude from plotting
    ...
    for step in tqdm.tqdm(range(niter)):
        ...
        al_hulls = trace_boundaries(wd, component_attr='components_grid', label_attr='labels_grid',
        ...
        if plot and (step % plot_every) == 0:
            ...
            ternary_fig = plot_ternary(working_dataset, ['c', 'a', 'b'], next_point=next_sample_dict, show=False,
                                       surface_colorbar=False, surface_data='labels_grid')
            for data in ternary_fig.data:
    ...
    ds_output['score_mean'] = (
        pd.DataFrame(results['score_mean']).to_xarray().rename(index='AL_step').to_array('phase').transpose('AL_step',
                                                                                                            ...))
    ds_output['score_std'] = (
