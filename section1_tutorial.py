...
from autophasemap import compute_elastic_kmeans, BaseDataSet
import os 
from scipy.signal import savgol_filter
from sklearn.cluster import kmeans_plusplus

boundary_path = os.getcwd()+"/challenge_datasets/tutorial.nc"
ref_path_sans = os.getcwd()+"/reference_sans/"
instrument = get_virtual_instrument(boundary_dataset_path = boundary_path, reference_data_path=ref_path_sans)
...
def label( dataset: xr.Dataset, num_phases: int ) -> xr.Dataset:
    ...
    _, random_sample = kmeans_plusplus(data.C, n_clusters=num_phases)
    clf = compute_elastic_kmeans(data,
                                 random_sample,
                                 max_iter=5,
                                 verbose=3,
                                 smoothen=False
...
AL_dataset = actively_learn(
    ...
    plot_every=2,
    plot_skip_phases=['D'],
    plot_save_path="./figures/tutorials/"
)